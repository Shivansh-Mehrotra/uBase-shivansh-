...
#sys.path.append('/home/devesh/projects/lal-path/NRL/script/layer/python')
...
def response(event, context):
    sr = StartResponse()
    output = application(environ(event, context), sr)
...
def environ(event, context):
    body = b''
    if event.get('body'):
        body = BytesIO(event.get('body').encode("utf-8"))
